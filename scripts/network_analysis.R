...
#      z>=2.5 = hub; standard P cut points for the 7 node roles. Small modules
#      flagged (size bias: Pedersen et al. 2020).
...
# ================== ALLEN CCF ANATOMICAL HIERARCHY LOOKUP ==================
# Maps region acronym -> major CCFv3 structure (annotation only).
allen_lookup <- c(
  FRP="Isocortex", MO="Isocortex", SS="Isocortex", GU="Isocortex",
  VISC="Isocortex", AUD="Isocortex", VIS="Isocortex", ACA="Isocortex",
  PL="Isocortex", ILA="Isocortex", ORB="Isocortex", RSP="Isocortex",
  PTLp="Isocortex", TEa="Isocortex", PERI="Isocortex", ECT="Isocortex",
  MOB="Olfactory", AOB="Olfactory", AON="Olfactory", TT="Olfactory",
  DP="Olfactory", PIR="Olfactory", COA="Olfactory", PAA="Olfactory", TR="Olfactory",
  CA="Hippocampus", DG="Hippocampus", FC="Hippocampus", IG="Hippocampus",
  ENT="Hippocampus", PAR="Hippocampus", POST="Hippocampus", PRE="Hippocampus",
  SUB="Hippocampus", ProS="Hippocampus", HATA="Hippocampus", APr="Hippocampus",
  CLA="Cortical Subplate", EP="Cortical Subplate",
  LA="Amygdala", BLA="Amygdala", BMA="Amygdala", PA="Amygdala",
  AAA="Amygdala", CEA="Amygdala", IA="Amygdala", MEA="Amygdala",
  CP="Striatum", ACB="Striatum", FS="Striatum", OT="Striatum",
  GPe="Pallidum/Septum", GPi="Pallidum/Septum", SI="Pallidum/Septum",
  MA="Pallidum/Septum", MS="Pallidum/Septum", NDB="Pallidum/Septum",
  TRS="Pallidum/Septum", LS="Pallidum/Septum", SF="Pallidum/Septum",
  SH="Pallidum/Septum", BST="Pallidum/Septum", BAC="Pallidum/Septum",
  VENT="Thalamus", SPF="Thalamus", SPA="Thalamus", PP="Thalamus",
  GENd="Thalamus", LAT="Thalamus", ATN="Thalamus", MED="Thalamus",
  MTN="Thalamus", ILM="Thalamus", RT="Thalamus", GENv="Thalamus", EPI="Thalamus",
  ASO="Hypothalamus", PVH="Hypothalamus", PVa="Hypothalamus", PVi="Hypothalamus",
  ADP="Hypothalamus", AVPV="Hypothalamus", DMH="Hypothalamus", MEPO="Hypothalamus",
  MPO="Hypothalamus", PD="Hypothalamus", PS="Hypothalamus", PVpo="Hypothalamus",
  SBPV="Hypothalamus", SFO="Hypothalamus", AHN="Hypothalamus", MBO="Hypothalamus",
  MPN="Hypothalamus", PMd="Hypothalamus", PVHd="Hypothalamus", PH="Hypothalamus",
  LHA="Hypothalamus", LPO="Hypothalamus", PST="Hypothalamus", PSTN="Hypothalamus",
  PeF="Hypothalamus", STN="Hypothalamus", ZI="Hypothalamus",
  SCs="Midbrain", IC="Midbrain", NB="Midbrain", SAG="Midbrain", PBG="Midbrain",
  MEV="Midbrain", SNr="Midbrain", VTA="Midbrain", PN="Midbrain", RR="Midbrain",
  MRN="Midbrain", SCm="Midbrain", PAG="Midbrain", PRT="Midbrain", CUN="Midbrain",
  RN="Midbrain", III="Midbrain", MA3="Midbrain", EW="Midbrain", IV="Midbrain",
  Pa4="Midbrain", VTN="Midbrain", AT="Midbrain", LT="Midbrain", DT="Midbrain",
  MT="Midbrain", SNc="Midbrain", PPN="Midbrain", RAmb="Midbrain",
  NLL="Pons", PSV="Pons", PB="Pons", B="Pons", DTN="Pons", PDTg="Pons",
  PCG="Pons", PRNc="Pons", SG="Pons", SUT="Pons", V="Pons", P5="Pons",
  Acs5="Pons", PC5="Pons", I5="Pons", CS="Pons", LC="Pons", LDT="Pons",
  NI="Pons", PRNr="Pons", RPO="Pons", SLC="Pons", SLD="Pons",
  AP="Medulla", CN="Medulla", DCN="Medulla", ECU="Medulla", NTS="Medulla",
  SPVC="Medulla", SPVI="Medulla", SPVO="Medulla", Pa5="Medulla", VI="Medulla",
  DMX="Medulla", GRN="Medulla", ICB="Medulla", IRN="Medulla", ISN="Medulla",
  MDRN="Medulla", PARN="Medulla", PAS="Medulla", PGRN="Medulla", PHY="Medulla",
  VNC="Medulla", x="Medulla", XII="Medulla", y="Medulla",
  LING="Cerebellum", CENT2="Cerebellum", CENT3="Cerebellum", DEC="Cerebellum",
  FOTU="Cerebellum", PYR="Cerebellum", UVU="Cerebellum", NOD="Cerebellum",
  SIM="Cerebellum", ANcr1="Cerebellum", ANcr2="Cerebellum", PRM="Cerebellum",
  COPY="Cerebellum", PFL="Cerebellum", FL="Cerebellum", FN="Cerebellum",
  IP="Cerebellum", DN="Cerebellum", VeCB="Cerebellum"
)
struct_of <- function(regions) {
  s <- allen_lookup[regions]; s[is.na(s)] <- "Unknown"; unname(s)
}
...
clean_and_matrix <- function(df, sex_label = NULL) {
  ...
  df_clean <- df_clean %>%
    mutate(across(-Region, ~ {
      x <- .
      neg_mask  <- !is.na(x) & x < 0
      zero_mask <- !is.na(x) & x == 0
      x[neg_mask] <- NA
      if (any(zero_mask)) {
        min_nonzero <- min(x[!is.na(x) & x > 0], na.rm = TRUE)
        if (is.finite(min_nonzero)) x[zero_mask] <- min_nonzero / 2 else x[zero_mask] <- NA
      }
      x
    })) %>%
    mutate(across(-Region, ~ log10(.)))
...
# Pedersen et al. 2020). Hub/non-hub cut = z_cut (Fos-scaled weighted WMDz, default 0.8).
...
run_density_sweep <- function(region_mat, label, densities = sweep_densities) {
  ...
  for (d in densities) {
    ...
    eff  <- mean(1 / offd[is.finite(offd) & offd > 0], na.rm = TRUE)
    trans_w <- mean(transitivity(g, type = "weighted"), na.rm = TRUE)
...
run_permutation_modularity <- function(region_mat, label, n_perm = 1000,
  ...
  data.frame(
    ...
    p_value_permutation = mean(Q_null >= Q_obs, na.rm = TRUE),
    n_permutations = n_perm, null_model = "degree_preserving",
    stringsAsFactors = FALSE
  )
}
...
for (comp in comparisons) {
  ...
  pdf <- permutation_metric_diff(mA, mB, comp[1], comp[2], n_perm = n_boot)
  if (!is.null(pdf)) permdiff_list[[paste(comp[1], comp[2])]] <- pdf
}
...
readme_sheet(wb1,
             ...
             c("Region x region Pearson correlation across animals (co-activation), one sheet per condition x sex.",
               "Pearson r on log10 (z-scored within sex for Combined) region profiles; this is the un-thresholded basis for all network construction.",
...
readme_sheet(wb2,
             ...
             c("Density-matched (proportional) thresholding on positive edges to a common density (primary_density) across all conditions; positive-only (Brynildsen 2020; Xie 2024); Rubinov & Sporns 2010; van den Heuvel 2017.",
               "One row per condition x sex: whole-network summary on the density-matched graph. N_components / Giant_frac report fragmentation after isolate removal; eigenvector centrality is computed on the giant component (off-giant nodes = NA).",
...
readme_sheet(wb3,
             ...
             c("Consensus Louvain communities detected on the thresholded graph itself (1000 runs; Lancichinetti & Fortunato 2012) - the graph-native partition for the cartographic metrics.",
               ...
               "Per region, proportion of conditions it stays in its modal network module (consensus modules).",
