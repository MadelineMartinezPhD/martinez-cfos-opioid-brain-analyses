...
# =============================================================================
# Figure 5, Panel C: force-directed co-activation network graphs, plus the
...
#
# Graph edges reproduce threshold_to_density() from Paper_Drug_Network_no_figs.R
...
##   Rscript scripts/network_graph_figures.R
## Override by editing the directory below or by setting the environment
## variable CFOS_OUTPUT_DIR before running.
...
out_dir    <- file.path(OUTPUT_ROOT, "figures", "Figure5C")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

font_fam <- "Arial"
dev_res  <- 1200
panel_in <- 69 / 25.4         # square network panels rendered at FINAL size (69 mm); place 1:1
density  <- 0.10              # primary_density (matches pipeline)
...
lab_cex    <- 1.0                            # region labels = 6 pt (device pointsize = 6, rendered
##                                              at final 70 mm and placed 1:1, so cex 1 == 6 pt).
...
## full 13-division palette (matches cartography_panels.R / Fig 4) - used by "division_fill"
div_order <- c("Isocortex", "Olfactory", "Hippocampus", "Cortical Subplate",
               "Amygdala", "Striatum", "Pallidum/Septum", "Thalamus",
               "Hypothalamus", "Midbrain", "Pons", "Medulla", "Cerebellum")
div_col <- c("#1B9E77", "#D95F02", "#7570B3", "#E7298A", "#66A61E", "#E6AB02",
             "#A6761D", "#666666", "#1F78B4", "#B2182B", "#FB9A99", "#6A3D9A",
             "#33A02C")
names(div_col) <- div_order
...
## cortex and nuclei; one judgment call, note in caption).
super_div <- c(
  ...
  "Pallidum/Septum" = "Cerebral nuclei", "Thalamus" = "Thalamus",
  "Hypothalamus" = "Hypothalamus", "Midbrain" = "Midbrain",
  "Pons" = "Hindbrain", "Medulla" = "Hindbrain", "Cerebellum" = "Cerebellum")
super_order <- c("Cerebral cortex", "Cerebral nuclei", "Thalamus", "Hypothalamus",
                 "Midbrain", "Hindbrain", "Cerebellum")
super_col <- c("Cerebral cortex" = "#1B9E77", "Cerebral nuclei" = "#D95F02",
               "Thalamus" = "#666666", "Hypothalamus" = "#1F78B4",
               "Midbrain" = "#D62728", "Hindbrain" = "#7570B3",
               "Cerebellum" = "#E6AB02")
...
build_graph <- function(tr, sx) {
  M   <- read_corr(tr, sx)
  adj <- threshold_to_density(M, density)
...
for (sx in sexes) for (tr in treatments) {
  ...
  tiff(paste0(stub, ".tiff"), width = panel_in, height = panel_in, units = "in",
       res = dev_res, compression = "lzw", family = font_fam, pointsize = 6); draw_network(g); dev.off()
  png(paste0(stub, ".png"),  width = panel_in, height = panel_in, units = "in",
      res = dev_res, family = font_fam, pointsize = 6); draw_network(g); dev.off()
...
draw_legend <- function() {
  ...
  if (color_mode == "division_fill") {
    ...
    text(0.02, 0.97, ttl, adj = 0, font = 2, cex = 1.0)
    ...
    for (i in seq_along(super_order)) {
      ...
      text(0.10, yy, super_order[i], adj = 0, cex = 1.0)
    }
    yh <- 0.90 - length(super_order) * 0.060 - 0.022   # extra gap so the thick hub ring
    ...
    if (color_mode %in% c("division_super", "group_fill")) {
      ...
      text(0.10, yh, "Hub (WMDz \u2265 1.5)", adj = 0, cex = 1.0)
      ybase <- yh - 0.07
  ...
  ## PC size (shared)
  text(0.02, ybase, "Participation Coefficient (PC)", adj = 0, font = 2, cex = 1.0)
  pcv <- c(0.0, 0.3, 0.6)
  for (i in seq_along(pcv)) {
    points(0.08 + (i - 1) * 0.12, ybase - 0.07, pch = 21, bg = "grey85",
           col = "grey40", cex = node_size(pcv[i]) / 2.55)   # dots sized to the 69 mm render's nodes (PC 0.6 dot == PC-0.6 node)
    text(0.08 + (i - 1) * 0.12, ybase - 0.12, pcv[i], cex = 1.0)  # number tucked under its dot (6 pt)
  }
...
png(.leg_tmp, width = 1.6, height = 2.2, units = "in",
    res = dev_res, family = font_fam, pointsize = 6); draw_legend(); dev.off()
