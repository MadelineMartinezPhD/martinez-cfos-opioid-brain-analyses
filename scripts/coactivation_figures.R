...
#
# Combined  -> main Figure 4
# Males / Females -> supplemental figures
#
# All outputs saved as BOTH .tiff and .png into a "Figure 4" folder,
# matching the per-figure folder convention used for Figures 2 and 3.
...
# Panels are written to a per-figure subfolder, matching Figures 2 and 3. The
...
# ---- Sizes (mm) & resolution ----------------------------------------
# RENDER AT FINAL SIZE. PNAS requires figures at final publication size
# with all text 6-12 pt AFTER any reduction; set MAT_MM to the size each
# matrix will actually occupy in the assembled figure, then place at 100%
# in Illustrator (no rescaling). Quick guide for a full-width (178 mm)
# Figure 4 with the two legends in a left column (~34 mm):
...
# ---- Type (points) ---------------------------------------------------
BASE_PS  <- 10
PT_TITLE     <- 8         # condition title above each heatmap (>= 6 pt at final size)
PT_KEY_TITLE <- 6 
PT_KEY_ITEM  <- 6  
PT_CB_TITLE  <- 6 
PT_CB_TICK   <- 7
...
# The values above define the main Figure 4 render. They are captured here so
...
LWD_AX  <- 0.6
PT_AXIS <- 7; PT_LAB <- 8; PT_LEG <- 6; PT_BAR <- 7; PT_CELL <- 7

# ---- Font (PNAS-approved: Arial / Helvetica; NOT Calibri) ------------
# PNAS only accepts Arial, Helvetica, Times, Symbol, Mathematical Pi,
# European Pi, and requires ALL figures in a submission to share one
# family. Your Figures 2/3 use Calibri, which PNAS substitutes in
# production -- for submission, re-render those in Arial too.
FONT_NAME <- "Arial"          # falls back to Helvetica, then device sans
...
DIFF_LIMIT    <- "auto"       # symmetric +/- limit: a number, or "auto" (quantile)
...
SEXDIFF_LIMIT      <- "auto"      # a number, or "auto" (quantile over the set)
...
render_colorkey <- function(family) {
  ...
  text(0.5, 0.998, "Color Key", font = 2, cex = ptcex(PT_KEY_TITLE),
       family = family, adj = c(0.5, 1))
  top <- 0.965; bot <- 0.00; gap <- 0.012   # title + 13 swatches all fit within the 59mm matrix: "Color Key" title top = matrix top, Cerebellum bottom = matrix bottom
...
render_corrbar <- function(family, lo = -1, hi = 1, label = "Correlation\nStrength",
  ...
  text(0.07, 0.5, ttl, font = 2, cex = ptcex(PT_CB_TITLE),
...
render_meanr <- function(family) {
  ...
  .y0 <- hi; .dy <- 0.05                                  # .y0 = 0.9 (dead level)
  for (i in 1:3) {
    yy <- .y0 - (i - 1) * .dy
...
render_involvement <- function(family) {
  ...
  # division colours are decoded by the shared Fig4_legend_ColorKey
}
...
main <- function() {
  ...
  if (DO_HEATMAPS) {
    ...
    for (g in groupings) {
      ...
      for (it in g_items) {
        base <- file.path(fig_dir, sprintf("Fig4_%s_%s_heatmap",
      ...
      local({
        ...
        save_both(function(fam) render_corrbar(fam, s$lo, s$hi, LOOK$label),
                  file.path(fig_dir, paste0("Fig4_legend_CorrelationStrength_", g)),
                  15, 65)
      })
    }
    save_both(render_colorkey, file.path(fig_dir, "Fig4_legend_ColorKey"), 26, 59)
      ...
      if (length(dmats) == 0) {
        ...
        for (i in seq_along(dmats)) {
          local({
            ...
            base <- file.path(fig_dir, sprintf("Fig4_Diff_%s_heatmap", slug(ttl)))
            save_both(function(fam) render_heatmap(m, ttl, fam, s$lo, s$hi,
        ...
        save_both(function(fam) render_corrbar(fam, sc$lo, sc$hi,
                  ...
                  file.path(fig_dir, "Fig4_legend_DiffStrength"), 15, 65)
      ...
      if (length(sdmats) == 0) {
        ...
        for (i in seq_along(sdmats)) {
          local({
            ...
            s <- sc; ordr <- ord_s
            base <- file.path(fig_dir, sprintf("Fig4_SexDiff_%s_heatmap", slug(sconds[i])))
            save_both(function(fam) render_heatmap(m, ttl, fam, s$lo, s$hi,
                                                   s$center, ordr, pal = DIFF_PALETTE), base, Wmm, Hmm)
          })
        }
        save_both(function(fam) render_corrbar(fam, sc$lo, sc$hi,
                  ...
                  file.path(fig_dir, "Fig4_legend_SexDiff"), 15, 65)
  ...
  if (DO_MEANR) {
    ...
    save_both(render_meanr, file.path(fig_dir, "Fig4_MeanAbsR_byCondition"), MR_W, MR_H)
  }
    ...
    if (is.null(items)) {
      ...
      if (all(is.na(M))) {
        ...
        save_both(render_divr,
                  file.path(fig_dir, "Fig4_Strength_byDivision"), DS_W, DS_H)
      }
    }
  }
      ...
      if (nrow(edges) < 1) {
        ...
        if (DO_INVOLVE) {
          ...
          save_both(render_involvement,
                    file.path(fig_dir, "Fig4_Involvement_byRegion"), IB_W, IB_H)
        }
        if (DO_DIVMAT) {
          ...
          save_both(render_divmat,
                    file.path(fig_dir, "Fig4_DivisionMatrix_Strengthened"), DM_W, DM_H)
        }
      }
    }
  }
...
#
# Each mode in RENDER_MODES is rendered in turn into its own folder. Figure 4 is
